...
Examples:
  ...
  spreaker chapter list 12345 --format json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			var episodeID int
			if _, err := fmt.Sscanf(args[0], "%d", &episodeID); err != nil {
	...
	listCmd.Flags().IntP("limit", "l", 20, "Maximum number of chapters")
	cmd.AddCommand(listCmd)
...
Examples:
  ...
  spreaker chapters add 12345 --starts-at 120000 --title "Main Topic" --url "https://example.com" --image chapter.jpg`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			var episodeID int
			if _, err := fmt.Sscanf(args[0], "%d", &episodeID); err != nil {
	...
	addCmd.Flags().Int("starts-at", -1, "Position in milliseconds (required)")
	addCmd.Flags().String("title", "", "Chapter title (required)")
	addCmd.Flags().String("url", "", "External URL")
	addCmd.Flags().String("image", "", "Image file path")
	addCmd.Flags().String("crop", "", "Crop coordinates: x1,y1,x2,y2")
...
Examples:
  ...
  spreaker chapters update 12345 67890 --image remove`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			var episodeID, chapterID int
			if _, err := fmt.Sscanf(args[0], "%d", &episodeID); err != nil {
				return fmt.Errorf("invalid episode ID: %s", args[0])
			}
			if _, err := fmt.Sscanf(args[1], "%d", &chapterID); err != nil {
				return fmt.Errorf("invalid chapter ID: %s", args[1])
			}
			...
			// Build params only with flags that were set
			params := api.ChapterParams{}
	...
	updateCmd.Flags().Int("starts-at", 0, "Position in milliseconds")
	updateCmd.Flags().String("title", "", "Chapter title")
	updateCmd.Flags().String("url", "", "External URL")
	updateCmd.Flags().String("image", "", "Image file path (or 'remove' to delete)")
	updateCmd.Flags().String("crop", "", "Crop coordinates: x1,y1,x2,y2")
...
Examples:
  ...
  spreaker msg list 12345 --format json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			var episodeID int
			if _, err := fmt.Sscanf(args[0], "%d", &episodeID); err != nil {
	...
	listCmd.Flags().IntP("limit", "l", 20, "Maximum number of messages")
	cmd.AddCommand(listCmd)
...
Examples:
  ...
  spreaker msg delete 12345 67890`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			var episodeID, messageID int
			if _, err := fmt.Sscanf(args[0], "%d", &episodeID); err != nil {
				return fmt.Errorf("invalid episode ID: %s", args[0])
			}
			if _, err := fmt.Sscanf(args[1], "%d", &messageID); err != nil {
				return fmt.Errorf("invalid message ID: %s", args[1])
			}
